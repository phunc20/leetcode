class Solution:
    ...
    def maxAbsoluteSum(self, nums):
        ...
        def min_subarray(L):
            """
            same as max_abs_subarray() but find min.
            """
            #j = len(L) - 1
            j = len(L)
            somme = sum(L)
        ...
        sup = max_subarray(list(reversed(nums[:max_subarray(nums)[0]])))[1]
        inf = min_subarray(list(reversed(nums[:min_subarray(nums)[0]])))[1]
