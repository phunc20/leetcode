...
def getBiggestThree(grid):
    ...
    def rhombus_sums(grid, row, col):
        ...
        while fit(m, n, row, col, side):
            ...
            ## debug
            try:
                for member in members(row, col, side):
                    sum_ += grid[member[0]][member[1]]
                print(f"sum_ = {sum_}")
            except:
                print(f"members(row, col, side) = {members(row, col, side)}")
                raise
    ...
    def fit(m, n, row, col, side):
        ...
        v_fit = row + 2*side < m
        if v_fit:
            print(f"fit({m,n,row,col,side}) still fits.")
            return True
        else:
            return False

    def members(row, col, side):
        #right_moves = [1] * side + [-1] * side
        #left_moves = [-1] * side + [1] * side
        #down_moves = [1] * (2*side)
        #L_right_members = [(row, col)]
        #for r_mv, d_mv in zip(right_moves, down_moves):
        #    new_member = (L_right_members[-1][0] + r_mv, L_right_members[-1][1] + d_mv)
        #    L_right_members.append(new_member)
        #L_left_members = [(row, col)]
        #for l_mv, d_mv in zip(left_moves, down_moves):
        #    new_member = (L_left_members[-1][0] + l_mv, L_left_members[-1][1] + d_mv)
        #    L_left_members.append(new_member)
        #return set(L_right_members) | set(L_left_members)
        ...
        for r_mv, d_mv in zip(right_moves, down_moves):
            #new_member = (row + r_mv, col + d_mv)
            new_member = (row + d_mv, col + r_mv)
            S_members.add(new_member)
        for l_mv, d_mv in zip(left_moves, down_moves):
            #new_member = (row + l_mv, col + d_mv)
            new_member = (row + d_mv, col + l_mv)
            S_members.add(new_member)
        print(f"S_members = {S_members}")
        return S_members
